...
Complete reference for constructing valid trace JSON for ingestion into the Experience Graph.
...
```bash
echo '{"source":"agent","intent":"test","context":{}}' | xpg ingest trace -
```

### With JSON Output

```bash
xpg ingest trace trace.json --format json
```
