...
- CLI (`xpg`) exists with ingest, curate, retrieve, admin command groups
...
1. **Ensure all operations are CLI-accessible**
   ...
   - Missing CLI commands to add:
     - `xpg retrieve lessons` - list promoted precedents (exists in MCP as `experience_lessons`)
     - `xpg retrieve cases` - list recent traces (exists in MCP as `experience_cases`)
     - `xpg curate feedback` - record feedback against a pack/trace
     - `xpg admin stats` - store counts and health metrics
...
2. **Add output filtering flags**
   - `--fields` flag on list/search commands to select specific fields
     - Example: `xpg retrieve search "dbt" --format json --fields summary,trace_id`
...
4. **Stdin support for batch operations**
   - `xpg ingest trace -` reads from stdin (already works)
   - `xpg ingest trace --batch` reads newline-delimited trace JSON
   - `cat traces/*.json | xpg ingest trace --batch`
...
- Agents can chain `xpg ... | jq ...` for precise context extraction
...
3. **Refactor MCP server**
   - Create `src/xpgraph/mcp/` package (move from single `mcp_server.py`)
...
1. **Create `src/xpgraph_sdk/` package**
   - `client.py` - `XPGClient` class
     - Constructor: `XPGClient(base_url=None, config_path=None)`
...
2. **Define skill functions (high-level wrappers)**
   - `src/xpgraph_sdk/skills.py`:
     ```python
     def get_context_for_task(client: XPGClient, intent: str, domain: str = None, max_tokens: int = 1500) -> str:
         """Returns a markdown summary of relevant context. Designed for LLM consumption."""

     def get_latest_successful_trace(client: XPGClient, task_type: str) -> str:
         """Returns markdown summary of the most recent successful trace matching task_type."""

     def save_trace_and_extract_lessons(client: XPGClient, trace: dict) -> str:
         """Ingests trace and runs precedent extraction. Returns summary."""

     def get_applicable_policies(client: XPGClient, domain: str) -> str:
...
3. **LangGraph tool adapter**
   - `src/xpgraph_sdk/integrations/langgraph.py`:
     ```python
     def create_xpg_tools(client: XPGClient) -> list[BaseTool]:
...
4. **CrewAI tool adapter**
   - `src/xpgraph_sdk/integrations/crewai.py`:
     - Same pattern, CrewAI `Tool` wrappers

5. **Claude Code skill**
   - Create an XPG skill for Claude Code that wraps CLI commands
   - Provides `xpg-context` skill that agents can invoke to get pre-summarized context
   - Uses CLI under the hood with `--format json | jq` for filtering

### Acceptance Criteria
- `XPGClient` works in both local and remote modes
...
2. **Token usage report**
   - `xpg analyze token-usage --days 7`
...
### Shared Formatters
Both the MCP Macro Tools (Phase 2) and the SDK skills (Phase 3) need the same summarization logic. The `formatters.py` module created in Phase 2 should live in core `xpgraph` (not in the MCP package) so the SDK can reuse it.

**Recommended location:** `src/xpgraph/retrieve/formatters.py`
