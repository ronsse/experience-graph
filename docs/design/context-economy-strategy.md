...
```bash
xpg retrieve search "dbt models" --format json | jq '.[].summary'
```
...
**What it is:** The `xpg` binary that directly interacts with the REST API or local SQLite databases.
...
**Agent Interaction (The Trick):** Do not expose the entire `xpg` raw API through MCP. Instead, build specific "Macro Tools" into the XPG MCP server that are designed to return summarized markdown, not raw JSON.
