...
1. **Extract formal ABCs from existing stores**
   ...
   - Create `src/xpgraph/stores/base/` package with pure ABCs:
   ...
   - Move SQLite implementations to `src/xpgraph/stores/sqlite/`
   - Keep backward-compatible imports in `stores/__init__.py`

2. **Create a StoreRegistry / DI container**
   - Add `src/xpgraph/stores/registry.py`
   - `StoreRegistry` class that:
     - Reads store backend config from `xpg.yaml` (e.g., `stores.graph.backend: "sqlite"`)
   ...
   - Example config:
     ...
     stores:
       graph:
         backend: sqlite
         path: ~/.xpg/data/stores/graph.db
       vector:
         backend: sqlite
         path: ~/.xpg/data/stores/vectors.db
     ```
...
3. **Refactor CLI and MCP server to use StoreRegistry**
   ...
   - CLI: inject via config loaded in `xpgraph_cli/config.py`
   - MCP server: replace `_get_*_store()` context managers with registry

4. **Add BlobStore abstraction**
   - Create `src/xpgraph/stores/base/base_blob.py` -> `BlobStoreBase(ABC)`
...
- `xpg admin init` creates config with default (sqlite) backends
...
1. **Create `src/xpgraph_api/` package**
   ...
   - Add `xpgraph_api` to pyproject.toml packages
...
2. **Add feedback ingestion endpoint**
   - `POST /api/v1/feedback` / `xpg ingest feedback`
...
3. **Context effectiveness query**
   - `xpg analyze context-effectiveness --days 30`
...
5. **CLI support**
   - `xpg retrieve entity <id> --as-of 2026-01-15`
   - `xpg retrieve pack --intent "..." --as-of 2026-01-15`
...
5. **Configuration**
   - `xpg.yaml` supports cloud backends:
     ```yaml
     stores:
       graph:
         backend: postgres
...
1. **Ingestion worker framework**
   - `src/xpgraph_workers/ingestion/base.py` -> `IngestionWorker(ABC)`
     - Methods: `discover()`, `extract()`, `load()`
     - Standardized entity/edge output format

2. **dbt manifest ingester**
   - `src/xpgraph_workers/ingestion/dbt.py`
...
3. **OpenLineage event ingester**
   - `src/xpgraph_workers/ingestion/openlineage.py`
   - Consumes OpenLineage JSON events
   - Creates entities for datasets and jobs
   - Creates edges for `reads_from` / `writes_to`

4. **CLI commands**
   - `xpg worker ingest-dbt <manifest-path>`
   - `xpg worker ingest-lineage <events-path>`
   - `xpg worker ingest-dbt --watch` (watches for manifest changes)
...
- `xpg retrieve search "model_name"` finds ingested models
