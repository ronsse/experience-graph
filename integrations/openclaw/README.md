# Experience Graph — OpenClaw Integration

Add structured institutional memory to your OpenClaw agents. Experience Graph provides traces, precedents, a knowledge graph, temporal versioning, and governed mutations — complementing OpenClaw's built-in memory with structured knowledge.
...
By default, XPG uses SQLite for all stores (zero configuration). For advanced setups, create `~/.xpg/config.yaml`:
...
See the [main repository](https://github.com/experience-graph/experience-graph) for full configuration options.
