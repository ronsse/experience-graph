...
- A running Postgres instance with DSN in XPG_TEST_PG_DSN env var
...
PG_DSN = os.environ.get("XPG_TEST_PG_DSN")
pytestmark = [
    pytest.mark.postgres,
    pytest.mark.skipif(PG_DSN is None, reason="XPG_TEST_PG_DSN not set"),
]
...
def _clean_tables(dsn: str) -> None:
    ...
    with conn.cursor() as cur:
        for table in ("traces", "documents", "nodes", "edges", "events"):
...
class TestPostgresEventLog:
    ...
    def _make_event(self) -> Event:  # noqa: F821
        from xpgraph.stores.base.event_log import Event, EventType
