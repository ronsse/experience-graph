...
def _inject_fake_boto3():
    ...
    if "xpgraph.stores.s3.blob" in sys.modules:
        del sys.modules["xpgraph.stores.s3.blob"]
    if "xpgraph.stores.s3" in sys.modules:
        del sys.modules["xpgraph.stores.s3"]
...
def store(mock_client):
    ...
    from xpgraph.stores.s3.blob import S3BlobStore
