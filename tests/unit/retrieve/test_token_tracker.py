...
from xpgraph.retrieve.formatters import auto_trim_response
from xpgraph.retrieve.token_tracker import estimate_tokens, track_token_usage
from xpgraph.retrieve.token_usage import TokenUsageReport, analyze_token_usage
...
class _FakeEventLog(EventLog):
    ...
    def count(
        ...
        *,
        ...
        return len(
            self.get_events(event_type=event_type, since=since)
        )
...
def test_auto_trim_low_relevance_strategy():
    ...
    result, trimmed = auto_trim_response(
        text, max_tokens=30, strategy="low_relevance"
    )
    assert trimmed is True
...
def test_auto_trim_low_relevance_single_section():
    text = "## Only section\n" + "x" * 500
    result, trimmed = auto_trim_response(
        text, max_tokens=20, strategy="low_relevance"
    )
    assert trimmed is True
