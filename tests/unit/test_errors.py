...
def test_xpgraph_error_has_message_and_code():
    err = XPGraphError("something broke", code="TEST_ERR")
...
def test_validation_error_is_xpgraph_error():
    err = ValidationError("bad input", errors=["field x required"])
    assert isinstance(err, XPGraphError)
