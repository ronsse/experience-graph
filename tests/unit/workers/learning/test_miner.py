...
from xpgraph.schemas.enums import OutcomeStatus, TraceSource
...
def _make_trace(
    *,
    ...
    outcome = (
        None
        if no_outcome
        else Outcome(status=status, summary=summary)
    )
...
async def _mock_llm(
    *,
    ...
    """Stub LLM that returns a well-formed JSON array."""
    return json.dumps([
        {
...
class TestExtractPrecedentHappy:
    ...
    def test_trace_not_found_returns_none(
...
class TestExtractEventEmission:
    ...
    def test_no_event_without_event_log(
        self, trace_store: SQLiteTraceStore
    ) -> None:
